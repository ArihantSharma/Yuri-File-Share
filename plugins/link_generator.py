...
async def batch(client: Client, message: Message):
    ...
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("😆 ꜱʜᴀʀᴇ ᴜʀʟ ʙᴜᴅᴅʏ", url=f'https://telegram.me/share/url?url={link}')]])
    await second_message.reply_text(f"<b>ʜᴇʀᴇ'ꜱ ʏᴏᴜʀ ʟɪɴᴋ ʙᴏꜱꜱ 😁</b>\n\n{link}", quote=True, reply_markup=reply_markup)
...
async def link_generator(client: Client, message: Message):
    while True:
        try:
            channel_message = await client.ask(text = "ꜰᴏʀᴡᴀʀᴅ ᴛʜᴇ ᴍᴇꜱꜱᴀɢᴇ ꜰʀᴏᴍ ᴅʙ ᴄʜᴀɴɴᴇʟ (ᴡɪᴛʜ Qᴜᴏᴛᴇꜱ)..\n\nᴏʀ ꜱᴇɴᴅ ᴛʜᴇ ᴅʙ ᴄʜᴀɴɴᴇʟ ᴘᴏꜱᴛ ʟɪɴᴋ", chat_id = message.from_user.id, filters=(filters.forwarded | (filters.text & ~filters.forwarded)), timeout=60)
        except:
            return
    ...
    link = f"https://t.me/{client.username}?start={base64_string}"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("🫥 ꜱʜᴀʀᴇ ᴜʀʟ ʙᴜᴅᴅʏ", url=f'https://telegram.me/share/url?url={link}')]])
    await channel_message.reply_text(f"<b>ʜᴇʀᴇ'ꜱ ʏᴏᴜʀ ʟɪɴᴋ ʙᴏꜱꜱ 😋</b>\n\n{link}", quote=True, reply_markup=reply_markup)
